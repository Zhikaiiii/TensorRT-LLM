...
def get_engine_name(model, dtype, world_size, rank):
    return '{}_{}_world_size{}_rank{}.engine'.format(model, dtype, world_size, rank)
...
def build_rank_engine(builder: Builder,
    ...
    with net_guard(network):
        ...
        network.set_named_parameters(
            tensorrt_llm_qwen.named_parameters())
...
def build(rank, args):
    args.output_dir = os.path.join(args.output_dir, args.dtype, f'tp{args.tp_size}_pp{args.pp_size}')
    ...
    for cur_rank in range(args.world_size):
        ...
        builder_config = builder.create_builder_config(
            ...
            tensor_parallel=args.tp_size,  # TP only
            pipeline_parallel=args.pp_size,
            parallel_build=args.parallel_build,
            num_layers=args.n_layer,
            num_heads=args.n_head,
            num_kv_heads=args.n_kv_head or args.n_head,
            hidden_size=args.n_embd,
            vocab_size=args.vocab_size,
            hidden_act=args.hidden_act,
            max_position_embeddings=args.n_positions,
